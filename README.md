...
**0) Prerequisites:**
- An up-to-date Docker instance (tested with `20.10.16)`.
- `git` installed.
- A stable internet connection.
- The OMOP vocabulary downloaded from Athena*, unzipped, and eventually complemented by proprietary vocabularies (e.g., CPT-4, cf. [https://github.com/hpi-dhc/athena-cpt4](https://github.com/hpi-dhc/athena-cpt4)).
...
Use *docker compose* to run all three services: a) The import script service, b) the *postgreSQL* database service, and c) the *pgAdmin* service:
...
**5) Uncomment data import service from *docker compose* file**

Before you start your containers again, make sure you uncomment the entire `import-data` service section (lines 4-14) from the *docker-compose.yml* (e.g., in VS Code `"Ctrl" + "/"` on Windows/Linux (`"Cmd" + "/"` on Mac respectively).
...
**IMPORTANT:** Make sure you have uncommented the entire `import-data` service section in the `docker-compose.yml` file as indicated in step 5 above.
